...
void ReadFasta(char candidatefilepath[], char output[],float kthreshould){
    int len;
    int cfaLen=0;
    char *pc;
    int i,j;
   ...
   //calculate the distance of seed sequence and candidate sequence.
    ...
    //struct point table[1000][1000];
    for (i=0;i<cfaLen;i++){
        if (strlen(candidate[i].sequence)<=1) break;
        for (j=i+1;j<cfaLen;j++){
            if (strlen(candidate[j].sequence)<=1) break;

            //calculate kmer distance at first
            kmerDis=kmerDistance(candidate[i].sequence,candidate[j].sequence);
            if (kmerDis<=kthreshould){
                rdistance=Init(candidate[i].sequence,candidate[j].sequence);
                fprintf(fout,"%s\t%s\t%f\n",candidate[i].name,candidate[j].name,rdistance);
            }
        }
    }
    free(pc);
...
void main(int argc, char *argv[]){
    "usage: xxx fasta/file/path output/file/path kthreshould";
    ReadFasta(argv[1],argv[2],atof(argv[3]));
}
